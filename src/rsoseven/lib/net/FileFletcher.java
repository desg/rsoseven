...
public class FileFletcher {
	 ...
	 * 
	 * Fletch data while leaving the exact same web client signature as the
	 * Jagex Client
	 */
	private URL fileURL;
	...
	public FileFletcher(URL fileURL, RSHeaders header) throws IOException {
		...
		if (header.getHeaders().get("accept-encoding") != null) {
			c.addRequestProperty("accept-encoding",
					header.getHeaders().get("accept-encoding"));
		}
		...
		DataInputStream httpInputstream = new DataInputStream(
				c.getInputStream());
