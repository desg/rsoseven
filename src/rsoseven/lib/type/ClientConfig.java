...
public class ClientConfig {
	...
	private static Pattern codeBaseRex = Pattern.compile("^codebase=(.*)$",Pattern.MULTILINE);
	private static Pattern jarNameRex = Pattern.compile("^initial_jar=(.*)$",Pattern.MULTILINE);
	private static Pattern classNameRex = Pattern.compile("^initial_class=(.*\\.class)$",Pattern.MULTILINE);
	private static Pattern paramsRex = Pattern.compile("^param=([0-9]+)=(.*)$",Pattern.MULTILINE);
	
	public ClientConfig(byte [] configbuffer){
		this.configText = new String(configbuffer);
		//I'm going in dry
		//System.out.println(configText);
		//^that is printed below, i use that text to match my shit on (i don't wanne load per line cus its multiline data)
