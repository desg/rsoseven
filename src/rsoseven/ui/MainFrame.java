...
public class MainFrame {
	...
	public MainFrame() throws AWTException, IOException,
		...
		mList.add(new Message(
				"This chat is _NOT_ part of runescape and is not official, endorsed, or moderated by JAGEX.",
				Color.DARK_GRAY));
		mList.add(new Message(
				"You are not allowed to use this client to infringe with the JAGEX Rules of Conduct.",
				Color.DARK_GRAY));
		mList.add(new Message(
				"Welcome to MM client V4, This client is not a cheating/botting tool, Please don't cheat!",
				Message.INFO));
		mList.add(new Message(
				"REPORT RUNESCAPE TOS VIOLATIONS TO: Nick.Hermans.Be@Gmail.com",
				Message.ALERT));
		mList.add(new Message(
				"To view HELP press CTRL+Q, To close this Box Press CTRL+C",
				Message.INFO));
		...
		Image i = new ImageIcon(getClass().getClassLoader().getResource(
				"res/icon.png")).getImage();
		frame.setIconImage(i);
		ImageIcon icon = new ImageIcon(getClass().getClassLoader().getResource(
				"res/loader.gif"));
		ImageIcon spacerIcon = new ImageIcon(getClass().getClassLoader()
				.getResource("res/spacer.png"));
		...
		mainFrameSize = new Dimension(sideDecorSize * 2 + RS_CLIENT_X,
				topDecorSize + sideDecorSize + RS_CLIENT_Y + 100);

		// add keylistener:
		GlobalScreen.registerNativeHook();
		GlobalScreen.getInstance().addNativeKeyListener(
				new KeyShortcutReader(this));
